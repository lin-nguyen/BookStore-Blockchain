...
from thuvienbk.solidity_parser.solidity_antlr4.SolidityLexer import SolidityLexer
from thuvienbk.solidity_parser.solidity_antlr4.SolidityParser import SolidityParser
from thuvienbk.solidity_parser.solidity_antlr4.SolidityVisitor import SolidityVisitor
...
class Node(dict):
    ...
    def accept(self,ob,ast,c): 
        if ast['type'] == 'SourceUnit': 
            return ob.visitSourceUnit(ast,c)
        if ast['type'] == 'PragmaDirective': 
            return ob.visitPragmaDirective(ast,c)
        if ast['type'] == 'ContractDefinition': 
            return ob.visitContractDefinition(ast,c)
        if ast['type'] == 'StateVariableDeclaration': 
            return ob.visitStateVariableDeclaration(ast,c)
        if ast['type'] == 'FunctionDefinition': 
            return ob.visitFunctionDefinition(ast,c)
        if ast['type'] == 'Block': 
            return ob.visitBlock(ast,c)
        if ast['type'] == 'IfStatement': 
            return ob.visitIfStatement(ast,c)
